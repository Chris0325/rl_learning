...
def dyna_q(episodes, n, alpha, epsilon, *, Q, action_space, nrow, ncol, p, s_begin, s_ends, γ=1):
    model, episode_steps = dict(), []
    for _ in range(episodes):
        ...
        while s not in s_ends:
            ...
            qs_hist = list(model)
            for q_index in np.random.choice(len(qs_hist), size=min(len(qs_hist), n)):
...
def run(runs, episodes, s_begin=s_begin, s_ends=[s_end]):
    for n in [0, 5, 50]:
        stats = np.zeros((runs, episodes))
        for i in tqdm(range(runs)):
            np.random.seed(0)
            Q = np.zeros((nrow, ncol, len(action_space)))
            stats[i] = np.array(dyna_q(episodes=episodes, n=n, alpha=.1, epsilon=.1, Q=Q, action_space=action_space, nrow=nrow, ncol=ncol, p=p, s_begin=s_begin, s_ends=s_ends, γ=.95))
